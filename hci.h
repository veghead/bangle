...
/* HCI Packet types */
// BLUETOOTH CORE SPECIFICATION Version 5.2 | Vol 4, Part A, Section 2
...
typedef struct {
    ...
    uint16_t    connection_interval_min[3];
    uint16_t    connection_interval_max[3];
    uint16_t    max_latency[3];
    uint16_t    supervision_timeout[3];
    uint16_t    min_ce_length[3];
    uint16_t    max_ce_length[3];
} __attribute__ ((packed)) le_extended_create_connection_cp_phys;
#define LE_EXTENDED_CREATE_CONN_CP_SIZE 25
...
#define EVT_LE_ENH_CONN_COMPLETE	    0x0a
